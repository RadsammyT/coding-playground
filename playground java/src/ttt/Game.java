...
public class Game {
	...
	public static int PlayerXInput() {
		...
		*/
		...
		while (true) {
			var index = InputUtils.readInt("X, enter a number on the grid: ");
			if (index > grid.grid.length - 1 || index < 0) {
				continue;
			} else
			if (grid.getCell(index).equals("X") || grid.getCell(index).equals("O")) {
				continue;
			} else {
				return index;
			}
	...
	public static int PlayerOInput() {
		...
		// return (Integer) null;
		while (true) {
			var index = InputUtils.readInt("O, enter a number on the grid: ");
			if (index > grid.grid.length - 1 || index < 0) {
				continue;
			} else if (grid.getCell(index).equals("X") || grid.getCell(index).equals("O")) {
				continue;
			} else {
				return index;
			}
